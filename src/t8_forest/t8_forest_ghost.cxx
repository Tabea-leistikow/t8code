                     ...
                     const t8_element_t *elem, t8_locidx_t element_index)
{
  ...
  if (elem_copy == NULL || level != copy_level ||
      ...
      ts->t8_element_get_linear_id (elem, level)|| real_subelement_siblings) {
    /* Add the element */
      ...
      for (iface = 0; iface < num_faces; iface++) {
        ...
        if (ghost_method == 0) {
          if (forest->is_transitioned) {
            /* In quadriliteral transitioned forests, every element has exactly one neighbor at each face 
             * In the hexahedral case subelements can have two subelement siblings as face neighbors. */
            if(ts->t8_element_is_subelement){
              if(iface < 4){
                num_face_children = t8_element_get_num_sibling_neighbors_at_face(elem, iface);
              }
            }
            else{
              num_face_children = 1;
          ...
          if (!is_atom && forest->is_transitioned != 1) {
             ...
             * and for subelements in the 3D hexahedral refinement. */
            if (!ts->t8_element_is_subelement (elem) ||
                (tree_class == T8_ECLASS_QUAD
                 && ts->t8_element_is_subelement (elem) && iface == 1) ||
                (tree_class == T8_ECLASS_HEX 
                 && ts->t8_element_is_subelement(elem) && iface == 4)) {
              ...
              /* For quadrilateral elements, faces 0 and 2 point to a sibling subelement that must be in the 
               * same mpi process. In hexahedral case, faces 0,...,3 point to sibling subelements. */
